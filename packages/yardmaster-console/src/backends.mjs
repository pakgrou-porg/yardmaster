...
async function probeOne(name, kind, base, timeoutMs) {
...
export async function probeBackends(rawText, { localEngineUrl, timeoutMs = DEFAULT_TIMEOUT_MS } = {}) {
  ...
  const seen = new Map(); // dedupKey -> { name, kind, base_url }
  for (const [pname, p] of Object.entries(doc.providers ?? {})) {
    if (typeof p.base_url === "string" && p.base_url) {
      const base = p.base_url.replace(/\/+$/, "");
      seen.set(dedupKey(base), { name: pname, kind: p.kind || "openai_compatible", base_url: base });
    }
  }
  if (localEngineUrl) {
    const base = localEngineUrl.replace(/\/+$/, "");
    const k = dedupKey(base);
    if (!seen.has(k)) seen.set(k, { name: "local-engine", kind: "local", base_url: base });
  }

  const results = await Promise.all(
    [...seen.values()].map(({ name, kind, base_url }) => probeOne(name, kind, base_url, timeoutMs)),
  );
