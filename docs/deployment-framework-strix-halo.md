...
| Harness page loads (and the setup/login redirect works) but the agent UI shows **"Disconnected"**, no sessions, and every `/api/*` call 403s | the same trust fence, but scoped to `/api` — dsh checks the exact `Host:` authority the browser sent, and the initial page GET is *not* gated the same way, so the page itself can load over an untrusted authority while the RPC bridge silently refuses every request. If you're browsing by a DNS/mDNS hostname (`http://framework:3080`) instead of the IP, set `YM_LAN_HOSTNAME=framework` (in addition to `YM_LAN_HOST`) and redeploy — or always use the IP. |
